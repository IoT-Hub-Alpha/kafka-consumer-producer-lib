...
def test_get_single_producer_returns_same_instance_for_same_config():
    p1 = KafkaProducerManager.get_single_producer(bootstrap_servers='kafka:9092', client_id='svc-a')
    p2 = KafkaProducerManager.get_single_producer(bootstrap_servers='kafka:9092', client_id='svc-a')
    assert p1 is p2


def test_get_single_producer_returns_different_instances_for_different_configs():
    p1 = KafkaProducerManager.get_single_producer(bootstrap_servers='kafka:9092', client_id='svc-a')
    p2 = KafkaProducerManager.get_single_producer(bootstrap_servers='kafka:9092', client_id='svc-b')
    assert p1 is not p2
