...
def test_subscribe_topics_accepts_valid_topics_and_delegates():
    ...
    assert wrapper._consumer.subscribed == [[KafkaTopics.telemetry_raw, KafkaTopics.telemetry_clean]]
...
def test_consumer_init_rejects_unknown_kwargs():
    with pytest.raises(ValueError, match='Unknown KafkaSettings fields: strange_option'):
        IoTKafkaConsumer(
            bootstrap_servers='kafka:9092',
            client_id='db-writer',
            group_id='writers',
